...
def main():
    ...
    # load family_metrics.json and group_metrics.json
    if os.path.exists('family_metrics.json'):
        with open('family_metrics.json', 'r') as f:
            family_metrics_history = json.load(f)
    
    if os.path.exists('group_metrics.json'):
    ...
    # Initialize models
    # Initialize models with better parameters
    # Initialize models and trainers as before
    family_model = MalwareGNN(
        num_node_features=14,
        hidden_dim=256,
        num_classes=num_family_classes,
    ...
    start_epoch = 40  # The epoch to resume from
    family_checkpoint_path = f'checkpoint_family_epoch_{start_epoch}.pt'
    group_checkpoint_path = f'checkpoint_group_epoch_{start_epoch}.pt'
    print('loaded model')
    
    print(family_metrics_history)

    family_model, family_trainer  = resume_from_checkpoint(
        family_checkpoint_path, family_model, family_trainer, start_epoch
    )
    
    group_model, group_trainer = resume_from_checkpoint(
        group_checkpoint_path, group_model, group_trainer, start_epoch
    )
    
    # Load the best metrics so far
    best_family_f1 = max([m['val']['overall']['f1'] for m in family_metrics_history])
    best_group_f1 = max([m['val']['overall']['f1'] for m in group_metrics_history])
    
    # Initialize patience counters
    family_patience = 0
    group_patience = 0
    ...
    for epoch in range(start_epoch, num_epochs):
        # Rest of the training loop remains the same as in your original code
        family_train_metrics = family_trainer.train_epoch(train_loader_family, family_weights)
        family_val_metrics = family_trainer.evaluate(val_loader_family, family_weights)
        family_novel_metrics = evaluate_novel_detection(family_trainer, val_loader_family, family_weights)
