...
def load_batch(batch_file: str, batch_size: int = 32, device: torch.device = torch.device('cpu')) -> Optional[DataLoader]:
    ...
    for graph in batch:
        try:
            # Assign binary label (malware = 1, benign = 0)
            # Specifically check for 'benign' string
            family = getattr(graph, 'family', None)
            # Consider both None and 'benign' as benign samples
            is_benign = (family is None or family == 'benign')
            graph.y = torch.tensor(0 if is_benign else 1, dtype=torch.long).to(device)
            
            # Debug info for first few graphs in first batch
            if batch_file.endswith('batch_0000.pt') and len(processed) < 5:
                print(f"Graph {len(processed)}: family={family}, assigned_label={0 if is_benign else 1}")
    ...
    labels = [g.y.item() for g in processed]
    num_benign = labels.count(0)
    num_malware = labels.count(1)
    print(f"\nLabel distribution in batch {os.path.basename(batch_file)}:")
    print(f"Benign (0): {num_benign} ({num_benign/len(labels)*100:.2f}%)")
    print(f"Malware (1): {num_malware} ({num_malware/len(labels)*100:.2f}%)")
...
def main():
    ...
    new_families = identify_new_families(train_loader, test_loader)
    print(f"New families in test: {new_families}")

    train_model(model, train_loader, val_loader, optimizer, criterion, device, epochs=50)

    new_family_batches = filter_new_family_batches(test_loader, new_families)
    new_accuracy, new_loss = evaluate_new_families(model, new_family_batches, criterion, device)
    print(f"Test Accuracy on New Families: {new_accuracy:.2f}%")
