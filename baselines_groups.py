...
class BaselineGNNBehavioral(torch.nn.Module):
    """Basic GNN for behavioral group classification."""
    def __init__(self, num_node_features: int, hidden_dim: int, num_behavioral_groups: int):
        super().__init__()
        self.num_behavioral_groups = num_behavioral_groups
        ...
        self.classifier = torch.nn.Linear(hidden_dim, num_behavioral_groups)
...
class BehavioralBaselineAnalyzer:
    def __init__(self, data_loader: TemporalMalwareDataLoader, device: torch.device):
        self.data_loader = data_loader
        self.device = device
        self.results = defaultdict(dict)
        
        # Store behavioral group information
        self.num_behavioral_groups = data_loader.num_known_groups
        logger.info(f"Initialized with {self.num_behavioral_groups} behavioral groups")
    ...
    def extract_graph_features(self, loader) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for batch in loader:
            batch_size = batch.group.size(0)  # Using group instead of y for labels
            for i in range(batch_size):
                ...
                group_labels.append(batch.group[i].item())  # Use group labels
        
        return np.array(features_list), np.array(group_labels)

    def evaluate_baseline_gnn(self, train_loader, val_loader, test_loader):
        """Evaluate baseline GNN model for behavioral group classification."""
        logger.info("Evaluating Baseline GNN for Behavioral Groups...")
        
        model = BaselineGNNBehavioral(
            num_node_features=14,
            hidden_dim=128,
            num_behavioral_groups=self.num_behavioral_groups
        ).to(self.device)
        ...
        for epoch in range(100):
            ...
            for batch in train_loader:
                ...
                output = model(batch)
                loss = criterion(output, batch.group)  # Use group labels
            ...
            with torch.no_grad():
                for batch in val_loader:
                    ...
                    val_labels.extend(batch.group.cpu().numpy())  # Use group labels
            ...
            if val_f1 > best_val_f1:
                best_val_f1 = val_f1
                patience_counter = 0
                torch.save(model.state_dict(), 'best_baseline_behavioral_gnn.pt')
            else:
                patience_counter += 1
        ...
        # Test evaluation
        model.load_state_dict(torch.load('best_baseline_behavioral_gnn.pt'))
        model.eval()
        test_preds = []
        test_labels = []
        ...
        with torch.no_grad():
            for batch in test_loader:
                ...
                test_labels.extend(batch.group.cpu().numpy())  # Use group labels
    ...
    def evaluate_traditional_ml(self, train_loader, val_loader, test_loader):
        """Evaluate traditional ML baselines for behavioral group classification."""
        logger.info("Evaluating Traditional ML Methods for Behavioral Groups...")
        ...
        # Random Forest for behavioral group classification
        rf = RandomForestClassifier(n_estimators=100, random_state=42)
        rf.fit(X_train_scaled, y_train)
        rf_preds = rf.predict(X_test_scaled)
        self.results['random_forest'] = self._compute_metrics(y_test, rf_preds)
        
        # Evaluate novelty detection methods for new behavioral groups
    ...
    def _evaluate_novelty_detectors(self, X_train, X_test, y_test):
        """Evaluate novelty detection methods for identifying new behavioral groups."""
        # Isolation Forest
    ...
    def _compute_novelty_metrics(self, y_true, y_pred) -> Dict:
        """Compute novelty detection metrics for new behavioral groups."""
        # Consider samples with groups >= num_known_groups as novel
        novel_mask = y_true >= self.data_loader.num_known_groups
    ...
    def run_all_baselines(self):
        """Run all baseline evaluations for behavioral group classification."""
        # Load data with use_groups=True to get behavioral group labels
        train_loader, _ = self.data_loader.load_split('train', batch_size=32, use_groups=True)
        val_loader, _ = self.data_loader.load_split('val', batch_size=32, use_groups=True)
        test_loader, _ = self.data_loader.load_split('test', batch_size=32, use_groups=True)
        
        # Check if GNN model exists
        if not Path('best_baseline_behavioral_gnn.pt').exists():
            self.evaluate_baseline_gnn(train_loader, val_loader, test_loader)
        else:
            logger.info("Loading existing behavioral GNN model...")
            model = BaselineGNNBehavioral(
                num_node_features=14,
                hidden_dim=128,
                num_behavioral_groups=self.num_behavioral_groups
            ...
            with torch.no_grad():
                for batch in test_loader:
                    ...
                    preds = output.argmax(dim=1)
                    test_preds.extend(preds.cpu().numpy())
                    test_labels.extend(batch.group.cpu().numpy())
        ...
        # Save results
        with open('behavioral_baseline_results.json', 'w') as f:
...
def main():
    ...
    analyzer = BehavioralBaselineAnalyzer(data_loader, device)
